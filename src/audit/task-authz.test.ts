...
async function main() {
  console.log(`\nTask authz + reassignment test → ${BASE}\n${'─'.repeat(60)}`)
  ...
  check('reassigned user (other branch) now sees the task', b2List.some((t: any) => t._id === taskId), `b2!=b1=${b2 !== b1}`)
  const ownerList = (await call('GET', '/tasks', undefined, ownerTok)).json.data || []
  check('previous assignee no longer sees it', !ownerList.some((t: any) => t._id === taskId))
  ...
  await User.deleteMany({ _id: { $in: [ownerId, otherId, b2Id] } })
  await disconnectDB()
  void Branch

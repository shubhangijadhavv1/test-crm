...
async function main() {
  ...
  const leaveDays = att.filter((a: any) => a.status === 'leave').length
  check('G2: attendance marked leave for both days', leaveDays === 2, `leave days=${leaveDays}`)

  // ---- cleanup ----
  await connectDB()
  await Category.deleteMany({ name })
  await Attendance.deleteMany({ userId: empId })
  await LeaveRequest.deleteMany({ userId: empId })
  await LeaveBalance.deleteMany({ userId: empId })

...
const ticks = (idle: number, active: number) => [
...
async function main() {
  ...
  await connectDB()
  const att = await Attendance.findOne({ userId: empId, date: new Date().toISOString().slice(0, 10) })
  const tickCount = await ActivityTick.countDocuments({ attendanceId: att?._id })
  ...
  await Screenshot.deleteMany({ attendanceId: att?._id })

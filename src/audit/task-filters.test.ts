...
async function main() {
  ...
  check('superadmin sees both new tasks', all.some((t: any) => t._id === t1) && all.some((t: any) => t._id === t2))

  // employee A sees only own
  const mine = (await call('GET', '/tasks', undefined, aTok)).data || []
  check('employee sees only own task', mine.some((t: any) => t._id === t1) && !mine.some((t: any) => t._id === t2))

  // assignee filter
  const byA = (await call('GET', `/tasks?assignee=${aId}`)).data || []
  check('assignee filter returns only that user', byA.every((t: any) => String(t.assigneeId?._id || t.assigneeId) === aId) && byA.some((t: any) => t._id === t1))

  // priority filter
  const crit = (await call('GET', '/tasks?priority=critical')).data || []
  check('priority filter returns only critical', crit.every((t: any) => t.priority === 'critical') && crit.some((t: any) => t._id === t1))

  // range=today returns the due-today task, hides the no-due one
  const todayList = (await call('GET', '/tasks?range=today')).data || []
  check('range=today includes due-today task, excludes no-due', todayList.some((t: any) => t._id === t1) && !todayList.some((t: any) => t._id === t2))

  // ---- cleanup ----
  await connectDB()
  await Task.deleteMany({ _id: { $in: [t1, t2] } })

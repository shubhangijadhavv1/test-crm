...
async function call(method: string, path: string, body?: unknown, tok = token): Promise<{ status: number; json: any }> {
...
async function main() {
  ...
  const b = (list.json.data || []).find((x: any) => x._id === branchId)
  ...
  const days: any[] = cal.json?.data?.days || []
  const stat = (d: number) => days.find(x => x.day === d)?.status

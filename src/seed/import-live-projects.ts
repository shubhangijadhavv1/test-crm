...
// name, url, tech, server, assignee, priority, status, qa1, qa2, due
type Row = [string, string, string, string, string, string, string, number, number, string]
const DATA: Row[] = [
...
async function main() {
  ...
  let seq = await Project.countDocuments({ type: 'live' })
  for (const [name, url, tech, server, assignee, priority, status, qa1, qa2, due] of DATA) {
    ...
    let proj = await Project.findOne({ name, type: 'live', isDeleted: false })
    if (proj) { Object.assign(proj, fields); await proj.save(); updated++ }
    else { seq++; proj = await Project.create({ ...fields, projectCode: `LIV-${String(seq).padStart(4, '0')}` }); created++ }

    // QA process with the QA1/QA2 percentages
    const state = qa1 >= 100 && qa2 >= 100 ? 'passed' : qa1 >= 100 ? 'stage2_inprogress' : 'stage1'
    const stage = (p: number) => ({ progress: p, status: p >= 100 ? 'done' : p > 0 ? 'inprogress' : 'notstarted', reviewerId: empCache.get(assignee) })
    await QaProcess.findOneAndUpdate(
      { projectId: proj._id },
      { projectId: proj._id, stage1: stage(qa1), stage2: stage(qa2), state, updatedBy: by },

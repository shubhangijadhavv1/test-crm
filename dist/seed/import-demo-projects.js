...
async function main() {
    ...
    let seq = await Project_1.Project.countDocuments({ type: 'demo' });
    for (const [name, url, tech, server, assignee, priority, statusText, due] of DATA) {
        ...
        const proj = await Project_1.Project.findOne({ name, type: 'demo', isDeleted: false });
        if (proj) {
            ...
            seq++;
            await Project_1.Project.create({ ...fields, projectCode: `DEM-${String(seq).padStart(4, '0')}` });
            created++;
        }
    }
...
//# sourceMappingURL=import-demo-projects.js.map
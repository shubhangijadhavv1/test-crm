...
exports.withImageSizes = withImageSizes;
/**
...
async function checkAvailability(url, timeoutMs = 12000) {
...
const between = (s, re) => { const m = s.match(re); return m ? (m[1] || '').trim() : ''; };
const all = (s, re) => { const out = []; let m; const r = new RegExp(re, 'gi'); while ((m = r.exec(s)))
    out.push(m[1] || m[0]); return out; };
const hasMeta = (s, prop) => new RegExp(`<meta[^>]+(property|name)=["']${prop}["']`, 'i').test(s);
/** Detect website technology from HTML signatures + headers. */
function detectTech(html, header) {
    const t = new Set();
    const h = (header('x-powered-by') || '') + ' ' + (header('server') || '');
    if (/wp-content|wp-includes|wp-json/i.test(html))
        t.add('WordPress');
    if (/\/_next\/|__NEXT_DATA__/i.test(html))
        t.add('Next.js');
    if (/data-reactroot|react(?:-dom)?(?:\.production)?/i.test(html))
        t.add('React');
    if (/__NUXT__|nuxt/i.test(html))
        t.add('Nuxt');
    if (/data-v-[0-9a-f]{8}|vue(?:\.runtime)?/i.test(html))
        t.add('Vue');
    if (/ng-version|angular/i.test(html))
        t.add('Angular');
    if (/php/i.test(h))
        t.add('PHP');
    if (/express|node/i.test(h))
        t.add('Node.js');
    if (/cloudflare/i.test(header('server') || ''))
        t.add('Cloudflare');
    if (!t.size)
        t.add('HTML');
    return [...t];
...
async function analyzePage(url, timeoutMs = 15000) {
    const started = Date.now();
    const ctrl = new AbortController();
    ...
    try {
        ...
        return {
            ...
            issues: [{ severity: 'critical', code: 'fetch_failed', message: e?.message || 'Fetch failed' }],
...
function analyzeHtml(html, ctx) {
    ...
    if (!title)
        add('high', 'missing_title', 'Page is missing a <title> tag');
    else if (title.length > 65)
        add('low', 'long_title', `Title is long (${title.length} chars)`);
    if (!metaDesc)
        add('high', 'missing_meta_description', 'Missing meta description');
    if (!canonical)
        add('medium', 'missing_canonical', 'Missing canonical tag');
    if (h1s.length === 0)
        add('medium', 'missing_h1', 'No H1 heading found');
    else if (h1s.length > 1)
        add('low', 'multiple_h1', `Multiple H1 tags (${h1s.length})`);
    if (!og)
        add('low', 'missing_og', 'Missing Open Graph tags');
    if (!viewport)
        add('medium', 'missing_viewport', 'Missing responsive viewport meta');
    if (jsonld.length && !jsonldValid)
        add('low', 'invalid_jsonld', 'JSON-LD structured data is invalid');
    // ---- links / images / assets ----
    const hrefs = all(html, /<a[^>]+href=["']([^"'#]+)["']/i);
    const internalSet = new Set(), externalSet = new Set();
    for (const href of hrefs) {
        if (/^(mailto:|tel:|javascript:|#)/i.test(href))
            continue;
        let abs;
        try {
            abs = new URL(href, finalUrl).toString().split('#')[0];
        }
        catch {
            continue;
        }
    ...
    for (const tag of imgTags) {
        ...
        if (!/^https?:/i.test(abs) || imgSeen.has(abs))
            continue;
        imgSeen.add(abs);
    ...
    if (imgsNoAlt)
        add('medium', 'img_alt', `${imgsNoAlt} image(s) missing ALT text`);
    if (mixed)
        add('high', 'mixed_content', `${mixed} insecure (http://) resource(s) on an HTTPS page`);
    if (!https)
        add('high', 'no_https', 'Site is not served over HTTPS');
    // ---- security headers ----
    const sec = {
    ...
    if (!sec.hsts && https)
        add('medium', 'no_hsts', 'Missing Strict-Transport-Security header');
    if (!sec.csp)
        add('medium', 'no_csp', 'Missing Content-Security-Policy header');
    if (!sec.xfo)
        add('low', 'no_xfo', 'Missing X-Frame-Options (clickjacking) header');
    if (!sec.xcto)
        add('low', 'no_xcto', 'Missing X-Content-Type-Options header');
    if (status >= 400)
        add('critical', `http_${status}`, `Page returned HTTP ${status}`);
    if (responseMs > 3000)
        add('medium', 'slow', `Slow server response (${responseMs} ms)`);
    // ---- scores (0–100), penalised per weighted issue ----
    const penalty = (codes, weights) => Math.max(0, 100 - issues.filter(i => codes.includes(i.code)).reduce((n, i) => n + (weights[i.severity] ?? 5), 0));
...
async function withImageSizes(images, cap = 60, opts = {}) {
    const list = images.slice(0, cap);
    const concurrency = opts.concurrency ?? 8;
    const timeoutMs = opts.timeoutMs ?? 6000;
    let i = 0;
    const out = list.map(im => ({ ...im }));
    async function worker() {
        while (i < list.length) {
            ...
            try {
                ...
                if (len)
                    out[idx].bytes = Number(len);
            }
            catch { /* leave bytes undefined */ }
            finally {
                clearTimeout(t);
            }
        }
...
//# sourceMappingURL=siteAnalyze.js.map
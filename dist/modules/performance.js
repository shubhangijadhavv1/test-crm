...
async function compute(scope, month) {
    ...
    const [users, branches, attendance, tasks, holidays, leaves] = await Promise.all([
        ...
        User_1.User.find({ isDeleted: false, role: { $ne: 'superadmin' }, ...scope }).select('fullName department role branchId behaviourScore avatarColor').lean(),
        ...
        Task_1.Task.find({ isDeleted: false, dueAt: { $gte: monthStart, $lte: new Date(Math.min(monthEnd.getTime() + 86400000, Date.now())) } }).select('assigneeId status').lean(),
    ...
    return users.map(u => {
        ...
        const parts = Object.keys(W).filter(k => applicable[k]);

...
async function audit(module, collection, fn) {
    try {
...
async function main() {
    ...
    await audit('Branch Management', 'branches', async () => {
        ...
        assert(list.json.data.some((b) => b.name === name), 'not found after create');
    ...
    await audit('Catalog — Categories', 'categories/subcategories', async () => {
        ...
        const found = list.json.data.find((x) => x._id === id);
    ...
    await audit('Catalog — Website Types', 'websitetypes', async () => {
        ...
        assert(list.json.data.some((t) => t._id === c.json.data._id), 'not found');
    ...
    await audit('Catalog — Servers', 'servers', async () => {
        ...
        const f = list.json.data.find((s) => s._id === c.json.data._id);
    ...
    await audit('QA & Checklists', 'qaprocesses', async () => {
        ...
        const qa = list.json.data.find((q) => q.projectId?._id === projId);
    ...
    await audit('Leave Management', 'leaverequests/leavebalances', async () => {
        ...
        assert(inbox.json.data.some((l) => l._id === c.json.data._id), 'not in inbox');
    ...
    await audit('Notice Board', 'announcements/announcementreads', async () => {
        ...
        const f = list.json.data.find((a) => a._id === id);

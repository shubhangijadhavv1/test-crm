...
async function main() {
    console.log(`\nScreenshot upload + static serving → ${BASE}\n${'─'.repeat(60)}`);
    ...
    check('screenshot appears in admin list', list.some((s) => s.url === url));
    // reject non-image
    ...
    const shotId = list.find((s) => s.url === url)?._id;
    ...
    check('deleted screenshot no longer listed', !list2.some((s) => s._id === shotId));
    check('deleted file removed from disk', !fs_1.default.existsSync(path_1.default.join(process.cwd(), url.replace(/^\//, ''))));
    // bulk delete: upload two, then remove both at once
    const up2 = async () => { const f = new FormData(); f.append('shot', new Blob([PNG], { type: 'image/png' }), 's.png'); const r = await fetch(BASE + '/agent/screenshot/upload', { method: 'POST', headers: { Authorization: `Bearer ${tok}` }, body: f }); return (await r.json()).data._id; };
    ...
    check('bulk-deleted screenshots gone from list', !list3.some((s) => s._id === a || s._id === b));
    // ---- cleanup ----
    await (0, db_1.connectDB)();
    const att = await Attendance_1.Attendance.findOne({ userId: empId });
    const filePath = path_1.default.join(process.cwd(), url.replace(/^\//, ''));
    try {
...
//# sourceMappingURL=agent-screenshot.test.js.map